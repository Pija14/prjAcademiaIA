...
(function () {
  ...
  const MUSCLE_PALETTE = ["#E1756E", "#F59F51", "#FCCD67", "#84CDA7", "#8FB0FC", "#8D82E8"];
  ...
  function renderMuscleChart(workouts) {
    ...
    const callouts = data.map((item, i) => {
      ...
      return `<path class="home-donut-callout-line" stroke="${color}" d="M ${x1.toFixed(2)} ${y1.toFixed(2)} L ${x2.toFixed(2)} ${y2.toFixed(2)} L ${x3.toFixed(2)} ${y3.toFixed(2)}"></path><text class="home-donut-callout-text" x="${(x3 + side * 2).toFixed(2)}" y="${(y3 - 1).toFixed(2)}" text-anchor="${anchor}">${percentage}%</text>`;
    }).join("");
    const legend = data.map((item, i) => `<div class="home-donut-legend-item"><span class="home-donut-legend-dot" style="background:${MUSCLE_PALETTE[i % MUSCLE_PALETTE.length]}"></span><span>${escapeHtml(item.label)}</span><b>${Math.round((item.value / total) * 100)}%</b></div>`).join("");
    requestAnimationFrame(() => {
      const draw = () => {
        try {
          const container = document.getElementById(chartId);
          ...
          chart.draw(table, { backgroundColor: "transparent", pieHole: 0.64, pieSliceText: "none", pieSliceBorderColor: "#FFFFFF", colors: data.map((_, i) => MUSCLE_PALETTE[i % MUSCLE_PALETTE.length]), legend: { position: "none" }, chartArea: { left: 8, top: 8, width: "84%", height: "84%" }, tooltip: { textStyle: { fontName: "Arial", fontSize: 12 } }, enableInteractivity: true, pieStartAngle: 0 });
        } catch (_) {}
      };
      ensureGoogleCharts().then(draw).catch(() => {});
    });
    return `<div class="home-google-donut-wrap"><div class="home-google-donut" id="${chartId}" role="img" aria-label="Distribuição de ${total} exercícios por grupo muscular"></div><svg id="${calloutId}" class="home-donut-callouts" viewBox="0 0 100 100" aria-hidden="true">${callouts}</svg><div class="home-google-donut-center" aria-hidden="true"><strong>${total}</strong><span>exercícios</span></div></div><div class="home-donut-legend" aria-label="Legenda da distribuição por grupo muscular">${legend}</div>`;
  }
