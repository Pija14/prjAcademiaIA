...
function renderSets(e){const count=exerciseSetCount(e);if(!count)return `<div class="empty">Configure a quantidade de séries antes de iniciar este exercício.</div>`;while(e.sets.length<count)e.sets.push({number:e.sets.length+1,reps:"",weight:"",done:false,completedAt:null});return e.sets.slice(0,count).map((s,i)=>`<div class="set-row"><span class="setnum">${i+1}</span><input class="set-reps" value="${esc(s.reps)}" placeholder="reps" disabled aria-label="Repetições da série ${i+1}"><input class="set-weight" value="${esc(s.weight)}" placeholder="kg" inputmode="decimal" onchange="setValue(${i},'weight',this.value)" aria-label="Carga em Kg da série ${i+1}"><span class="check ${s.done?'done':''}">${s.done?'✓':'○'}</span></div>`).join("");}
